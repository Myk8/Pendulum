...
namespace Pendulum
{
    public partial class Foform : Form
    {
        ...
        static Dictionary<string, string> nez = new Dictionary<string, string>();

        private void jegyzet()
        {
            nez.Add("Hold Your Colour", "hold_your_colour");
            nez.Add("In Silico", "in_silico");
            nez.Add("Immersion", "immersion");
        ...
        static string connectionString = @"Server = (localdb)\MSSQLLocalDB;" +@"AttachDbFileName=|DataDirectory|adatbázishoz\music.mdf;";
        ...
        private List<string[]> dgwtartalek = new List<string[]>();
        private void cBAlbum_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (nez.ContainsKey(cBAlbum.SelectedItem.ToString()))
                pictureBox.Image = (Image)Resource1.ResourceManager.GetObject(nez[cBAlbum.SelectedItem.ToString()]);
            connection.Open();
            SqlDataReader command = new SqlCommand($"select Albums.release, Tracks.length from Albums , Tracks where Albums.id = Tracks.album and Albums.title like '{cBAlbum.SelectedItem}';", connection).ExecuteReader();
            int seconds = 0;
            string date = "";
            while (command.Read())
            {
            ...
            while (command1.Read())
            {
                ...
                dgwtartalek.Add(new string[] { command1[0].ToString(), command1[1].ToString() });
            }
            connection.Close();
        ...
        private void tBSearch_TextChanged(object sender, EventArgs e)
        {
            ...
            foreach (string[] item in dgwtartalek)
            {
        ...
        public void Formbezar(object sender, FormClosingEventArgs e)
        {
            DialogResult dialogResult = MessageBox.Show("Biztosan így szeretni menteni?", caption: CaptionButton.Close.ToString(), buttons: MessageBoxButtons.YesNo);
            if (dialogResult == DialogResult.Yes)
            {
                ...
                connection.Close();
                connection.Open();
                SqlDataReader command2 = new SqlCommand($"select Albums.release, Tracks.length from Albums , Tracks where Albums.id = Tracks.album and Albums.title like '{cBAlbum.SelectedItem}';", connection).ExecuteReader();
                int seconds = 0;
                string date = "";
                while (command2.Read())
                {
